...
$\overline { { a } }$ 和 $\bar { b }$ 分别为两组数据的平均值； $\delta _ { a } \delta _ { b }$ 分别为对应的标准偏差。  
...
由于目前吸积盘粘滞度的起源和特性都还不清楚，用磁流体力学对其讨论是一种常用的方法，文[26]提出如果磁场的逃逸率比较低，可以认为 $\mu = 0 . 5$ ，使用这个参数值，长周期爆发时间大约为 $2 t _ { b u r s t }$ ，即
