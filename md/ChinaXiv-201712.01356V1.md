...
(1）可通过批量导入接口直接导入已采集的信息。但导人之前需要人工下载导入模板，按模板要求严格进行数据预处理，实现已有数据信息和CSpace元数据字段的映射。
...
该方法一般需要通过大量的样本积累和训练，或者由人工预先标注好一定数量的样本实例，交给机器程序去聚类、归纳学习，生成网页分类器(算法和规则)，利用分类器对网页信息进行模式处理。如Mitra等[8利用预定义的标签集合对DOM树节点进行训练,生成分类器；王浩[提出了将采样技术和半监督学习相结合的方法，对传统的 SMOTE 文本分类算法进行改进以实现网络敏感信息的识别；Pavlinek等[1o提出了基于主题模型表示的半监督式文本分类方法，该方法包括一个基于自训练的半监督文本分类算法和模型，用于识别和确定新文本内容的参数设置。
...
![](images/7f07b03a75be8157a36fa91460eb34961dcd2a6e31022d0d5e1e1a722cca3769.jpg)  
图2采集内容规则的配置流程
...
![](images/de0deeb0322c51aaf0fd59b4ed2323f2253bb6dbaed04f5e5b40a0371031ffcc.jpg)  
图3多级循环模式的列表页采集策略
...
A海事与航空部 □ □ 品 采网/  
海洋保护 □□ □□ □口 武urias_rs □ 回 □ 级藏：分类网t/eHurias_ulios □ □ □ 级第分页网// 4□国 □□ □□rSein,J □ □ D 共有记录需集  
美国国家大气与海 □ □ 品 成采集更：titO 国□ 国□ 国□□□M_leseurch √ □ 式功采集并更数据t据：tt://oracu/rs/artchlrrtalypiachecht  
...
Repository tor Data and Knowledge Services [R/OL]. (2016-10-17).[2017-07-17]. http://ir.las.ac.cn/handle/8879.)   
...
Wang Silil,²Liu Wei1Zhu ZhongminglWu ZhiqianglWang Jinping1 1(Lanzhou Literature and Information Center, Chinese Academy of Sciences, Lanzhou 73oooo, China) 2(University of Chinese Academy of Sciences,Beijing 10o049, China)
