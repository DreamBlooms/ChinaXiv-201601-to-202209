...
c)聚群行为。设鱼群个体当前位置为 $X _ { i }$ ，搜索邻域内的鱼群个体总数为 $n _ { f } ,$ 中心位置为 $X _ { c }$ 。若 $F ( X _ { c } ) / n { \scriptstyle \geqslant } d e l t a ^ { \ast } F ( X _ { i } )$ ，则$X _ { c }$ 处的食物浓度较高且不太拥挤。根据公式(5)进行移动，否则，执行觅食行为。
