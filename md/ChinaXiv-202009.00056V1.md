...
目前，两台Ⅱ型多功能天文经纬仪已经开展了一年多的后续观测实验，单台仪器一个晴夜进行4个小时的观测，观测约120颗恒星，采集图像数据量约为250GB，所有的图像被存储在图像处理工作站的磁盘阵列中。观测完成后，由值班观测人员利用项目组所开发图像处理软件进行图像批量自动处理。图4为主光路图像采集系统的软件框架。
