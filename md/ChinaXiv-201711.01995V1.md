...
基于模板的方法也是常被采用的方法之一。基于模板的方法一般会先建立模板数据库，然后通过查找和匹配模板，完成待匹配引文的抽取。Day 等[5统计出计算机科学领域科技文献的6种主要参考文献格式，构建了多层知识表示框架INFOMAP，并以此为基础，开发了基于知识的引文元数据抽取系统，其实质是基于多层模板的元数据抽取方法。Cortez 等提出一种无监督的引文元数据抽取方法，利用已存在的给定领域元数据作为训练集，自动生成模板完成抽取工作。Huang等7和Chen等[8使用蛋白质序列表示引文字符串，将引文模板的序列表示形式存人DNA数据库。然后借助在DNA 数据库中进行相似性比较的分析工具BasicLocalAlignmentSearchTool(BLAST)，为待分析的引文寻找相似的DNA，即引文模板，最后根据匹配的模板解析引文数据。
...
![](images/4553e5cad9c6474f35c2d41bfd078b13fb56da99fd2e5c67f0d5d801368c4771.jpg)  
图1技术路线图
...
![](images/6c63dfe0d8e6fe2d886aeb2a5cb80b9a29acc74ffad0ac0847b21f2c7d53e040.jpg)  
图25个词在两个向量空间中的位置  
图3训练集标注示例
...
31Reprinted in Interpretive Social Science   
41Berkeley   
...
![](images/24674908053c1f52f0bb7719d11560f6f02113ac042838c82d1c0d8edd46576c.jpg)  
图4CBOW模型网络结构
...
![](images/dfc866ff375e036175bd663cb7bc4f9765185256c1bfd3a7fd6c588f7a511a6e.jpg)  
图5分类特征训练说明图
...
SVM是机器学习研究中的一项重大成果，是一种重要的分类算法。它主要用于解决二值分类的模式识别问题。SVM是在统计学习理论(StatisticalLearningTheory，SLT)的基础上发展出来的一种新的通用学习方法，其核心内容是 Stitson 等[15]在1992 年到1995 年间提出的。采用支持向量机方法的主要优点是：
...
场模型和最大熵模型来解决序列化标注问题，当前普遍使用的模型是条件随机场模型(CRF)。为了能够可以更加凸显实验的效果，笔者使用条件随机场模型作为参照组进行了对比实验,CRF 是由 Lafferty 等[1提出,结合了最大熵模型和隐马尔可夫模型特点的一种无向图模型，近年来在分词、词性标注和命名实体识别等序列标注任务中取得了很好的效果，
...
对 2000 篇引文数据进行人工标注，标注的具体形式如表4所示。
...
![](images/903891628a55ea04fe333f867196e4b23cdef5d71c863d3249f9ce29af3cf750.jpg)  
具体的对比实验效果如图7所示。  
图7对比实验结果参数图
...
[2]Wei W,King I,Lee JH M. Bibliographic Atributes Extraction with Layer-upon-Layer Tagging[C]//Proceedings of the 9th International Conference on Document Analysis and Recognition. IEEE,2007,2: 804-808.   
...
[5]Day MY, Tsai R T H,Sung C L,et al. Reference Metadata Extraction Using a Hierarchical Knowledge Representation Framework [J]. Decision Support Systems，2007，43(1): 152-167.   
...
[9]Han H, Giles C L,Manavoglu E,et al.Automatic Document Metadata Extraction Using Support Vector Machines[C]// Proceedings of the 20o3 Joint Conference on Digital Libraries.IEEE,2003:37-48.   
...
[14]周练.Word2Vec 的工作原理及应用探究[J]．科技情报开发 与经济,2015 (2):145-148.(Zhou Lian.Exploration of the Working Principle and ApplicationofWord2Vec [J].Sci-Tech Information Development & Economy,2015 (2):145-148.)   
