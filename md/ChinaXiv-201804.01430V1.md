...
当且仅当 $\scriptstyle { s = 0 }$ 时，有 $s { \dot { s } } = 0$ 。
...
其中， $M ( q ) = \left[ \begin{array} { c c } { { 0 . 1 + 0 . 0 1 \cos ( q _ { 2 } ) } } & { { 0 . 0 1 \sin ( q _ { 2 } ) } } \\ { { 0 . 0 1 \sin ( q _ { 2 } ) } } & { { 0 . 1 } } \end{array} \right]$
