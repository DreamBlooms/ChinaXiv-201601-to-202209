...
经巴特勒球形检验，其数值为814.265,显著水平0.000，说明这项数据可以用作因子分析，如表4所示：
...
通过伪平均值与标准差的比较，判定每种用户类别的Web20行为使用程度。具体做法为：根据聚类分析的结果，将原先的497样本进行分组，在每个组内计算其平均数与标准差。分为：数值大于2的高度使用、数值大于1的中度使用、数值低于0的低度使用，如表13所示：
