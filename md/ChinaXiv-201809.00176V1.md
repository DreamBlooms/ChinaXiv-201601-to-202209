...
定理 $2 ^ { [ 1 8 ] }$ 进一步假设 $\psi ( t ) t$ 在 $t \in \left[ 0 , + \infty \right)$ 上是凸函数.
...
故由（3）（4）及文[18］中引理3的（i）（ii）和（ii）可得 $\psi \left( \left| u ^ { \left( q \right) } \left( x \right) \right| \right) \left| u ^ { \left( m - n \right) } \left( x \right) \right|$
