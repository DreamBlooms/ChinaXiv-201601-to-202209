...
在改进的GSA算法中，将粒子视为抗体，因此本文将粒子的适应度函数定义为粒子的亲和度函数，定义如下：
...
a)初始化参数。其中包括种群规模 $N$ ，算法最大迭代次数$T$ ，随机初始化粒子群体位置 $X _ { i }$ ，引力常量 $G _ { 0 }$ ，引力系数衰减因子 $\alpha$ 中各个参数，最优粒子复制数 $M$ 。
...
综上所述，对于文中选取的12个基准测试函数，本文提出的A2F-GSA算法具有最优的求解精度、收敛速度和鲁棒性。
