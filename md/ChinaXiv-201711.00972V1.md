...
在CATIA中建立地形模型可以利用自身的网格模型建立、编辑的功能实现，也可导入其他软件建立的地形模型文件4，下面介绍几种常用的CAT-IA地形模型的建立方法。
...
(3)建立起、终点路基标准断面，并将其与不含起、终点端面的全线路基标准轮廓曲面接合得到一完整的、闭合的全路段路基标准轮廓曲面模型。
...
# MethodsforRoad3D Modelingbased on CATIA
